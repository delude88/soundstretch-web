...
class Float32ChannelTransport implements ChannelTransport<Float32Array[]> {
  ...
  write(input: Float32Array[], offset: number = 0): void {
    ...
    if (input.length > 0 && input[0].length > 0) {
      ...
      for (let c = 0; c < channelCount; ++c) {
        const length = Math.min(input[c].length, this._sampleSize - offset)
  ...
  read(target?: Float32Array[], offset: number = 0, length: number = 0): Float32Array[] {
    const output = target || new Array(this._channelCount).map(() => new Float32Array(this._sampleSize))
    if (offset > this._sampleSize) throw new Error('Invalid offset')
    console.log(output)
    if (output.length > 0) {
      ...
      for (let c = 0; c < channelCount; ++c) {
        const bla = output[c].length
        const start = c * this._sampleSize
        const end = start + (length > 0 ? length : Math.min(output[c].length - offset, this._sampleSize))
